...
public class PagoService {
    ...
    public PagoResponse procesarPago (PagoRequest pago){
        ...
        if (montoPago <= 0) throw new PagoNoEncontradoException("El monto de pago debe ser mayor a cero para validar descuento");
    ...
    public PagoResponse reembolsarPago (Long idPago){
        Pago pago = pagoRespository.findById(idPago).orElseThrow(() -> new PagoNoEncontradoException("Pago no encontrado"));
        if (pago.getEstadoPago() != Estado.PAGADO) throw new PagoNoReembolsadoException("Solo se puede reembolsar un pago ya pagado");
